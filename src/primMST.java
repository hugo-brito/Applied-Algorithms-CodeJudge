...
public class primMST {
	...
	public primMST(Map<Integer,Set<Edge>> vertices, int m){

		this.mst = new HashSet<>(vertices.keySet().size() - 1);
		this.weight = 0;

		boolean[] visited = new boolean[vertices.keySet().size()];
		visited[0] = true;


		PriorityQueue<Edge> currentEdges = new PriorityQueue<>(m);
		currentEdges.addAll(vertices.get(0));

//		while (visited.size() < vertices.keySet().size()) {
//		while (mst.size() < vertices.keySet().size() - 1) {
		while (!currentEdges.isEmpty()) {

			Edge minEdge = currentEdges.remove();
			...
			if (!visited[w]) {
				visited[w] = true;
				for (Edge e : vertices.get(w)) {
					if (!visited[e.other(w)])
						currentEdges.add(e);
				}
			}

			this.weight += minEdge.weight;
